...
8. `apps/mobile/components/issue/sub-issue-row.tsx`：子 issue 行（`IssueRow` + 子进度角标 + 跳转）。
9. `apps/mobile/components/issue/parent-issue-row.tsx`：父 issue 行（状态图标 + 标识 + 标题 + 跳转）。
10. `apps/mobile/components/issue/sub-issues-section.tsx`：区块（空态入口 / header / 分组列表 / 折叠）。
11. `apps/mobile/components/issue/timeline-list.tsx`：`ListHeader` 插入父 issue 行与子 issue 区块。
12. `apps/mobile/app/(app)/[workspace]/new-issue.tsx`：可选 `parent` 参数 → `parent_issue_id` + 缓存失效。
13. `apps/mobile/app/(app)/[workspace]/issue/[id].tsx`：下拉刷新带上子 issue 查询。
...
`data/api.ts` / `data/queries/*`，删除新增的 6 个文件即可。
