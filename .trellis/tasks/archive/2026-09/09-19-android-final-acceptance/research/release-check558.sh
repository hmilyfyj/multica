#!/usr/bin/env bash
# FEATURE-558 · Release 产物核验（生产变体，FEATURE-552 的签名 APK）
#
...
# 内容与 551 的 `release-check.sh` 一致，只把观测库换成 lib558.sh；生产包与 staging Debug
# 包能共存这件事由两者包名不同直接给出：
#   staging Debug = com.ehaier.zgq.shop.mall.staging，生产 Release = com.ehaier.zgq.shop.mall。
#
